...
| `dry_run` | When enabled, the step detects changes, runs security checks, and creates a local commit, then validates the push with `git push --dry-run` instead of performing a real `git push`.  If the dry-run push indicates that a real push would fail (e.g. authentication error, non-fast-forward), the step fails. Otherwise it exits with success (instead of the usual intentional failure) so the build is not affected.  Note: `AUTOFIX_PUSHED` will always be `false` in dry run mode.  | required | `false` |
